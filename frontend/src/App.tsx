...
function App() {
  ...
  const categories = [
    { id: "employee", title: "Staff Attendance", icon: Users, color: "bg-blue-500" },
    { id: "student", title: "Student Attendance", icon: GraduationCap, color: "bg-green-500" },
    { id: "vehicle", title: "Vehicle Attendance", icon: Car, color: "bg-purple-500" },
  ];
      ...
      <header className={`fixed top-0 w-full ${darkMode ? "bg-gray-800" : "bg-white"} shadow-md z-10`}>
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            {selectedCategory && (
              <button onClick={() => setSelectedCategory(null)} className="p-2 rounded-full hover:bg-gray-100">
                <ArrowLeft className={darkMode ? "text-white" : "text-gray-700"} />
              </button>
            )}
            <h1 className={`text-xl font-bold ${darkMode ? "text-white" : "text-gray-800"}`}>Seedo AI</h1>
          </div>
