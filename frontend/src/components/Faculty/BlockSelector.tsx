...
interface BlockSelectorProps {
  uniqueBlocks: { block: string; count: number }[];
  onBlockSelect: (block: string | null) => void;
  darkMode: boolean;
}

export const BlockSelector: React.FC<BlockSelectorProps> = ({ uniqueBlocks, onBlockSelect, darkMode }) => {
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {uniqueBlocks.map(({ block, count }) => (
          <button
            key={block}
          ...
          >
            ...
            <p className={`${darkMode ? "text-yellow-400" : "text-blue-400"} text-xl font-semibold`}>
              {count} Faculty {count === 1 ? "Member" : "Members"}
            </p>
          </button>
        ))}
      </div>
